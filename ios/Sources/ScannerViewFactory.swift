...
private final class ScannerPreview:UIView,AVCaptureVideoDataOutputSampleBufferDelegate,@unchecked Sendable{
    ...
    func update(_ values: [String: WireValue]) {
        queue.async { [weak self] in
            ...
            if changedFacing && self.configured { self.configure() }
            else { self.applyTorch() }
        }
    }
    ...
    private func send(_ values: [String: WireValue]) {
        guard lifetime.isActive, let data = try? WireMap.encode(values) else { return }
