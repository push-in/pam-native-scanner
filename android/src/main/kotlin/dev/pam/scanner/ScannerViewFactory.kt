...
private class ScannerHost(context: Context) : FrameLayout(context) {
    ...
    private var provider:ProcessCameraProvider?=null;private var camera:Camera?=null;private var scanner:BarcodeScanner=BarcodeScanning.getClient(options("1"));private var facing=1L;private var torch=false;private var duplicateMillis=1500L;private var formatSpec="1";private val seen=RecentScans()
    @Volatile private var released = false
    @Volatile private var enabled = true
    @Volatile private var bindingGeneration = 0
    ...
    fun update(values: Map<String, WireValue>) {
        if (released) return
        val nextFacing = values.integer("facing", 1)
        val nextFormats = values.text("formats", "1")
        val nextEnabled = values.flag("enabled", true)
        val rebind = nextFacing != facing || nextFormats != formatSpec || nextEnabled != enabled
        facing = nextFacing
        torch = values.flag("torch", false)
        enabled = nextEnabled
        duplicateMillis = values.integer("duplicateIntervalMillis", 1500).coerceIn(0, 60_000)
        if (nextFormats != formatSpec) {
            formatSpec = nextFormats
            scanner.close()
            scanner = BarcodeScanning.getClient(options(formatSpec))
        }
        if (!enabled) {
            ++bindingGeneration
            unbindCamera()
    ...
    private fun bind() {
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            send(mapOf("event" to WireValue.Integer(2), "message" to WireValue.Text("Camera permission is required")))
            return
        }
        val owner = context as? LifecycleOwner ?: run {
            send(mapOf("event" to WireValue.Integer(3), "message" to WireValue.Text("Scanner host has no lifecycle")))
            return
        }
        val future = ProcessCameraProvider.getInstance(context)
        future.addListener({
            if (!released && enabled && generation == bindingGeneration) {
                runCatching {
                    ...
                    send(mapOf("event" to WireValue.Integer(3), "message" to WireValue.Text(it.message.orEmpty())))
                }
            }
    ...
    private fun analyze(proxy: androidx.camera.core.ImageProxy) {
        if (released || !enabled || !processing.compareAndSet(false, true)) {
            proxy.close()
            return
        }
        val generation = bindingGeneration
        ...
        try {
            scanner.process(InputImage.fromMediaImage(image, proxy.imageInfo.rotationDegrees))
                .addOnSuccessListener { barcodes ->
                    if (released || !enabled || generation != bindingGeneration) return@addOnSuccessListener
                    val now = android.os.SystemClock.elapsedRealtime()
                    barcodes.forEach { barcode ->
                        val value = barcode.rawValue.orEmpty()
                        if (seen.accept(value, now, duplicateMillis)) {
                            send(mapOf(
                                "event" to WireValue.Integer(1),
                                "value" to WireValue.Text(value),
                                "format" to WireValue.Integer(format(barcode.format)),
                                "valueKind" to WireValue.Integer(valueKind(barcode.valueType)),
                            ), generation)
                        }
                    }
                }
                .addOnFailureListener {
                    send(mapOf("event" to WireValue.Integer(3), "message" to WireValue.Text(it.message.orEmpty())), generation)
                }
            ...
            processing.set(false)
            proxy.close()
            send(mapOf("event" to WireValue.Integer(3), "message" to WireValue.Text(error.message.orEmpty())), generation)
        }
    }
    ...
    private fun send(values: Map<String, WireValue>, generation: Int = bindingGeneration) = post {
        if (!released && enabled && generation == bindingGeneration) emitter?.invoke(WireMap.encode(values))
    }
